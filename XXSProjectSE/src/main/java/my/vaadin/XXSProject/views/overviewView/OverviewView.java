...
//		this.addComponent(centeringLayout);
	...
	private VerticalLayout buildUIExercises() {
					...
					if (selectedPlan != null) {
						...
						if (this.existingExercises != null && this.existingExercises.size() != 0) {
							for (Exercise exExer : this.existingExercises) {
								if (exExer.getName().equals(newExercise.getName())
										&& exExer.getFkWorkoutplanName().equals(newExercise.getFkWorkoutplanName())) {
									exerciseAlreadyExists = true;
								}
							}
						}

						// Übung wird nur hinzugefügt, wenn es sie im Plan noch
						// nicht gibt
						if (!exerciseAlreadyExists) {
							...
							Notification.show("Übung bereits in Plan enthalten", "",
		...
		this.btnRmvExercise.addClickListener(e -> {
			...
			if (selectedExercise != null) {
				...
				logDeleter.deleteLogsForUsernameAndWorkoutPlanAndExercise(selectedExercise.getFkUsername(),
						selectedExercise.getFkWorkoutplanName(), selectedExercise.getName());
		...
		contentLayoutExercises.addComponent(tfAddExerciseTargSets);
	...
	private VerticalLayout buildUIWorkout() {
		...
		this.gridWorkouts.addSelectionListener(e -> {
			...
			if (selectedPlanName != null) {
				...
				// Exercise-Grid laden
				this.existingExercises = exerciseProvider
						.getExercicesForUsersWorkoutPlan(this.getParentUI().getLoggedInUsername(), selectedPlanName);
		...
		this.btnAddWorkoutPlan.addClickListener(e -> {
			// Validitätscheck der eingegebenen Daten
			if (!this.tfAddWorkoutPlanName.getValue().equals("") && !this.tfAddWorkoutPlanDesc.getValue().equals("")) {
				newWorkout = new WorkoutPlan(tfAddWorkoutPlanName.getValue(), tfAddWorkoutPlanDesc.getValue(),
		...
		contentLayoutWorkout.addComponent(tfAddWorkoutPlanName);
		contentLayoutWorkout.addComponent(tfAddWorkoutPlanDesc);
		contentLayoutWorkout.addComponent(btnAddWorkoutPlan);
		contentLayoutWorkout.addComponent(btnRmvWorkoutPlan);

		return contentLayoutWorkout;
	}
	...
	public void enter(ViewChangeEvent event) {
		...
		this.existingWorkouts = workoutProvider.getWorkoutplansForUsername(this.getParentUI().getLoggedInUsername());
